...
const TechStackIcon = ({ TechStackIcon, Language, paddingPx = 0 }) => {
  const resolvedSrc =
    typeof TechStackIcon === 'string' &&
    !TechStackIcon.startsWith('http') &&
    !TechStackIcon.startsWith('/')
      ? `/${TechStackIcon}`
      : TechStackIcon;
  ...
  return (
    <div className="group p-6 rounded-2xl bg-slate-800/50 hover:bg-slate-700/50 transition-all duration-300 ease-in-out flex flex-col items-center justify-center gap-3 hover:scale-105 cursor-pointer shadow-lg hover:shadow-xl">
      <div className="relative">
        ...
        <div className="relative h-16 w-16 md:h-20 md:w-20" style={{ padding: paddingPx }}>
          <img
            src={resolvedSrc}
            alt={`${Language} icon`}
            className="h-full w-full object-contain transform transition-transform duration-300"
          />
        </div>
      </div>
